...
const RestaurantScreen = () => {
    ...
    const { params: {
        imgUrl, title, rating, genre, address, short_description, dishes
    } } = useRoute()
    ...
    return (
        <ScrollView>
            <View className="flex-row h-40 relative bg-gray-400">
                <View>
                    <TouchableOpacity className="bg-gray-100 top-20 left-4  p-3 rounded-full items-center" onPress={navigation.goBack}>
                        <ArrowLeftIcon size={20} color="#00ccbb" />
                    </TouchableOpacity>
                    <Image source={{ uri: imgUrl }} className="w-full" />
                </View>
            </View>
